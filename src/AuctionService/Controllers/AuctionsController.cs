...
public class AuctionsController : ControllerBase
{
    ...
    public async Task<ActionResult<AuctionDto>> CreateAuction(CreateAuctionDto auctionDto)
    {
        ...
        aucton.Seller = User.Identity.Name;

        _context.Auctions.Add(aucton);

        var newAuction = _mapper.Map<AuctionDto>(aucton);
        ...
        return CreatedAtAction(nameof(GetAuctionById), new { aucton.Id }, newAuction);
    ...
    public async Task<ActionResult> DeleteAuction(Guid id){
        ...
        await _publishEndpoint.Publish<AuctionDteleted>(new { Id = auction.Id.ToString() });
        var result = await _context.SaveChangesAsync() > 0;
